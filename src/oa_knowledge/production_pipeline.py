...
class ProductionQueue:
    ...
    def fail(self, task_id: int, owner: str, error_code: str, detail: str, *, recoverable: bool = True) -> None:
        with Session(self.engine) as session:
            ...
            transient_retry = recoverable and error_code in {
                "PIPELINE_RESOURCE_BUSY", "BROWSER_CLOSED", "OA_TIMEOUT",
            }
            can_retry = auth_retry or transient_retry or recoverable and row.attempts < row.max_attempts
            ...
            retry_minutes = 1 if auth_retry else min(30, 2 ** min(row.attempts, 5))
