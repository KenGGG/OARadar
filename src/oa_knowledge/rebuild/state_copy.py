...
def _reject_symlink_path(path: Path) -> Path:
    ...
    for component in lexical.parts[1:]:
        ...
        if current.is_symlink():
            raise ValueError("database paths must not contain symlinks")
    return lexical
...
def validate_database_copy(database_path: Path) -> list[ValidationCheck]:
    ...
    return [
        ...
        ValidationCheck("COPY_PROVENANCE", prepared_copy, 1, int(prepared_copy)),
...
def backup_live_database(source: Path, target: Path) -> None:
    ...
    source = _reject_symlink_path(source)
    target = _reject_symlink_path(target)
    if source == target:
    ...
    if any(sidecar.exists() for sidecar in _sqlite_sidecars(target)):
        raise ValueError("database copy target must not have SQLite sidecars")
