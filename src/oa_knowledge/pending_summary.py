...
from oa_knowledge.db.models import ItemSnapshot, LogicalItem, SummaryJob, SummaryVersion
...
def summarize_pending(settings: Settings, engine, logical_item_id: int) -> SummaryVersion:
    with Session(engine) as session:
        ...
        existing = session.scalar(select(SummaryVersion).where(
            ...
            SummaryVersion.input_hash == snapshot.content_hash, SummaryVersion.status == "current",
        ))
        if existing:
            return existing
        idem = f"pending:{logical_item_id}:{snapshot.content_hash}:{settings.llm.model}:pending-v1"
