...
_CATEGORY_PATTERNS: tuple[tuple[BusinessCategory, tuple[str, ...]], ...] = (
    ...
    ("02_业务项目与投放租后", (r"租赁项目", r"项目(?:立项|预审|评审|投放|合同)", r"投放条件", r"租后(?:检查|报告|管理)", r"直接租赁", r"售后回租", r"联合承租")),
    ("03_风险合规审计法务", (r"风险(?:专题|管理|审查)?", r"合规", r"审计", r"法务", r"诉讼")),
    ("04_财务资金与融资", (r"银行授信", r"授信(?:额度|材料|申请)", r"融资材料", r"资金管理", r"财务", r"银行账户", r"(?:明细账|总账|账务资料)", r"(?:提取|协定|通知|定期)存款", r"债务合同限制性.*填报")),
    ("05_经营计划与绩效考核", (r"经营计划", r"绩效(?:考核|评价)", r"年度考核", r"经营分析")),
    ("06_人力资源", (r"企业年金", r"人力资源", r"人员任免", r"招聘", r"薪酬", r"员工培训", r"岗位", r"人事", r"员工")),
    ("07_党建纪检与工会", (r"党建", r"党支部", r"党委", r"纪检", r"纪委", r"工会")),
    ("08_行政采购与信息化", (r"档案整理", r"档案(?:分类|归档范围|保管期限)", r"行政管理", r"办公用品", r"采购", r"信息化", r"信息系统", r"(?:软件|开票).*许可", r"软件服务", r"营业执照(?:原件)?(?:外借|借用)?")),
    ("09_对外报送与监管反馈", (r"监管(?:报送|反馈)", r"对外报送", r"统计报送", r"监管机构")),
)

_TITLE_SUBJECT_RULES: tuple[tuple[BusinessCategory, tuple[str, ...]], ...] = (
    ("01_公司治理与决策", (r"董事会", r"股东(?:会|大会)", r"监事会", r"公司治理", r"公司章程")),
    ("04_财务资金与融资", (r"融资(?:计划|方案)",)),
    ("07_党建纪检与工会", (r"工会(?:员工)?活动",)),
    ("09_对外报送与监管反馈", (r"(?:报|报送|反馈|回复).*(?:金服集团|集团|监管|政府|主管部门)", r"(?:金服集团|监管|政府|主管部门).*(?:报送|反馈|回复)", r"(?:监管|对外|统计)报送")),
    ("04_财务资金与融资", (r"银行授信", r"授信(?:额度|材料|申请)", r"融资材料", r"资金划转", r"银行账户", r"(?:电子银行|网银)", r"银行.*(?:协议|保密)", r"(?:明细账|总账|账务资料)", r"(?:提取|协定|通知|定期)存款", r"债务合同限制性.*填报")),
    ("05_经营计划与绩效考核", (r"(?:重点)?督办(?:项目|事项)?", r"重点任务跟踪")),
    ("02_业务项目与投放租后", (
        r"租后(?:检查|报告|管理)", r"资产检查", r"资产分类", r"项目(?:立项|预审|评审|投放|合同)", r"项目.*保密协议", r"内部尽职调查(?:请示|申请)?", r"项目[^。\n]{0,40}尽职调查的?内部请示",
        r"(?:直租|直接租赁)", r"售后回租", r"联合承租", r"业务方案变更", r"租赁项目",
    )),
    ("03_风险合规审计法务", (r"风险(?:专题|管理|审查)?", r"合规", r"审计", r"法务", r"法律顾问", r"诉讼", r"撤诉")),
    ("06_人力资源", (r"企业年金", r"人力资源", r"人员任免", r"招聘", r"薪酬", r"员工培训", r"岗位", r"人事", r"员工")),
    ("07_党建纪检与工会", (r"党建", r"党支部", r"党委", r"纪检", r"纪委", r"工会")),
    ("08_行政采购与信息化", (r"档案整理", r"档案(?:分类|归档范围|保管期限)", r"行政管理", r"办公用品", r"采购", r"信息化", r"信息系统", r"(?:软件|开票).*许可", r"软件服务", r"营业执照(?:原件)?(?:外借|借用)?")),
    ("05_经营计划与绩效考核", (r"经营计划", r"发展定位", r"发展目标", r"经营规划", r"绩效(?:考核|评价)", r"年度考核", r"经营分析", r"工作简报")),
)
...
def _subject_match(
    ...
    # Procedure references are not the subject of an otherwise ordinary document.
