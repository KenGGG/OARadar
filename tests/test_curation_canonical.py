...
def test_formal_canonical_path_uses_issuer_year_number_and_title() -> None:
    ...
    assert relpath.as_posix() == "curated/oa/正式文件/示例集团/2026/示例发〔2026〕12号_关于测试_安全工作的通知"
...
def test_internal_and_project_paths_follow_business_taxonomy() -> None:
    ...
    assert publication_relpath(internal, fallback_date="2026-08-15", collision_key="x").parts[2:5] == ("公司内部", "预算管理", "2026-08")
    assert publication_relpath(project, fallback_date="2026-08-15", collision_key="x").parts[2:6] == ("项目资料", "示例客户", "示例项目", "投标")
